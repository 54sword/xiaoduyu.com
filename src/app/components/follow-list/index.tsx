...
export default function(props:Props) {
  ...
  return (<ListClass
    {...props}
    {...list}
    load={(params: any)=>findFollows(params)(store.dispatch, store.getState)}
    renderItem={(item: any)=>{
      ...
      if (people) {
        ...
        content = (<div styleName="people-item">
          ...
          <div className="d-flex justify-content-between">
            <div>
              <Link to={`/people/${people._id}`} className="text-dark"><b>{people.nickname}</b></Link>
      ...
      if (posts) {
        ...
        content = (<div>
          <div className="d-flex justify-content-between">
            <div>
              <Link to={`/posts/${posts._id}`} className="text-dark"><b>{posts.title}</b></Link>
            </div>
            <div>
              <Follow posts={posts} />
            </div>
      ...
      if (topic) {
        ...
        content = (<div>
          <div className="d-flex justify-content-between">
            <div>
              <Link to={`/topic/${topic._id}`} className="text-dark"><b>{topic.name}</b></Link>
            </div>
            <div>
              <Follow topic={topic} />
            </div>
      ...
      return (<div className="card mb-0 border-0 border-bottom" key={key}>
